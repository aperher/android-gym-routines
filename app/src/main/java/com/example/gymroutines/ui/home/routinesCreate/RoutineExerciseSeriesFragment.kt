...
class RoutineExerciseSeriesFragment : Fragment(R.layout.fragment_exercise_series) {
    ...
    private fun initListeners() {
        ...
        binding.tvExerciseInfo.text =
            viewModel.activeExercise.value!!.equipment.value + ". " + viewModel.activeExercise.value!!.primaryMuscles.value
    }
    ...
    private fun setSeries() {
        for (i in 0 until binding.linearLayoutSeries.childCount) {
            val linearLayoutChild = binding.linearLayoutSeries.getChildAt(i) as? LinearLayoutCompat
            val textReps = linearLayoutChild?.getChildAt(1) as? TextInputEditText
